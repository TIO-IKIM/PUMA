...
from datasets.wsi import WSI, WSIMetadata
from utils.pp_tools import get_bounding_box
from utils.tools import remove_small_objects
...
class DetectionCellPostProcessorCupy:
    ...
    def _prepare_pred_maps(self, predictions_: dict) -> np.ndarray:
        ...
        predictions["nuclei_type_map"] = np.asarray(predictions["nuclei_type_map"].detach().cpu())
        predictions["nuclei_binary_map"] = np.asarray(predictions["nuclei_binary_map"].detach().cpu())
        predictions["hv_map"] = np.asarray(predictions["hv_map"].detach().cpu())
